...
% 	\end{proof}
 ...
 If $a(x),b(x)\in \fps{\Rsemiring}{x_\Sigma}$ are algebraic, then so are their sum $a(x)+b(x)$, multiplication $a(x)b(x)$, composition $a(b(x))$ and derivative $a'(x)$ \cite{DBLP:series/tmsc/KauersP11}. Recall from Section 4 that $a(b(x))$ is well-defined thanks to the continuity of $\Rsemiring$ (composition of algebraic power series is not generally well-defined over arbitrary (semi)rings). 
