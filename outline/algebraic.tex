% !TEX root = main.tex
  ...
  Algebraic power series are widely used in algebra and combinatorics, in the context of rings or fields. Its treatment in semirings is less standard, but extensively studied in the context of formal language theory.
...
As a famous example, consider again the fps $c(x)= \sum_{n \geq 0} C_n x^{n} \in \fps{\R}{x}$. One can verify that $c(x)$ is the Taylor expansion around $0$ of the (analytic) function $\frac{1-\sqrt{1-4x}}{2x}$. From this, it is easy to see, taking $p(w, x)= xw^2 - w + 1$, that $p(c(x), x)=0$. Observe that, while computing a value like $c(1/4) = \sum_{n \geq 0} \frac{1}{4^n(n+1)} \binom{2n}{n}$ as a limit is hard, we can easily deduce that $c(1/4)=1$ is the (only) positive root of $p(w,1/4)$.
