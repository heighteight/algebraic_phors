...
We thus need to \emph{impose} a form of finiteness, via the following technical definitions:


\begin{definition}[variable and family restrictions]
Let $s_X\in \fps{\Rsemiring}{\Sigma}^X$ be a family of fps, and let $\Sigma'\subset \Sigma$ and $X'\subset X$. Then:
\begin{varitemize}
...
%obtained by restricting $s_{X'}$ to the variables in $Y'$;
 \end{definition}
 Intuitively, in $s_\sigma\vert_{\Sigma'}$ we only keep those monomials $(s_\sigma)_\mu x^\mu$ from $s$ such that $\mu$ has support included in $\Sigma'$, and we 
 ``kill'' all monomials $(s_\sigma)_\mu x^{\mu}$ containing 
 ...
 Moreover, in $s\vert_{\Sigma'}^{X'}$ we furthermore ``kill'' all $s_\sigma$ with $\sigma\notin X'$.
	...
	\begin{definition}[finitary fps]\label{def:finitary}
		Let $s_X\in \fps{\Rsemiring}{X+Y}^Z$ be a family of fps. A pair $(U\subseteq X,W\subseteq Z)$ is called:
	\begin{varitemize}
