...
A similar argument, applied to Example \ref{ex:phors3}, shows that the solution $a_\star(z)\in \fps{\Rinf}{z}$ is algebraic over $\fps{\mathbb Q^+}{z}$.
  ...
  we also get:
   ...
   \end{corollary}
   Now assume that $X=\ X_1\to X_2 \bydef !X_1 \times X_2$ is an exponential object. In this case, we can consider  $eval(\fix s_X) = (\sum_{\kappa \in !X_1} r_{\kappa, x_2}(x_Y) x_{X_1}^\kappa)_{x_2 \in X_2}\in \fpp{\Rsemiring}{X_1 + Y}^{X_2}$. We have that this is an algebraic power series and, if $X_2$ is an exponential object itself, this can be iterated, always getting algebraic power series.
   	\begin{lemma}
   If $s_X\in \fps{\Q}{(X_1 \to X_2)+Y}^{!X_1 \times X_2}$ is a family of algebraic power series such that all but finitely many $x \in !X_1 \times X_2 \: s_x =0$.   Then $eval(s_X)$ is a family of $\Q$-algebraic power series with parameters $x_{X_1}, x_Y$.  
   \end{lemma}  
   \begin{proof}
