...
If $s_X$ is finitary, then one can find a finite sub-family $s_{A}$, for some $A=I(\sigma_X,X')\subset X$ such that, for all $a\in A$, the iterates $r^n_a$ (cf.~Eq.~\eqref{eq:fixpointeq}) are constructed using only polynomials from $s_A$. The fixpoint $\fix s_X=\sup_n r^{n}_X$ will be a solution of a fas generated from this family:
