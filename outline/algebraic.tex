...
%  As we saw, power series are infinitary objects and their manipulation can then involve the concept of limit, making it generally uncomputable. Moreover, the continuous semiring we will generally use in the context of probabilistic semantics will be $\Rinf$ and the semiring of formal power series over $\Rinf$: hence, the computation with the coefficients of power series themselves could be infinitary. To our rescue, we will use a way to give a finitary specification of power series: this will be the concept of algebraicity. 
  ...
  \begin{definition}
  	...
  	\end{cases}
  	\end{equation*}
  	 If for an $s \in \fps{R}{\Sigma}$, $(s)$ is an algebraic family (i.e there exists a polynomial $p(w_1, x_\Sigma) \in \fpp{R'}{w \cup \Sigma}$ such that $p(s, x_\Sigma)=0$, we say that $s$ is algebraic; we also say that $p$ is an annihilating polynomial of $w_1$.
  \end{definition}
...
%  		\end{itemize}
  ...
  \begin{remark}
  	...
  	If $s\in \fpp{\R}{x}$ has annihilating $p(w, x_\Sigma)$ is one of its annihilating polynomials, then one can compute effectively a rational power series $r(x,y)$ such tha the following holds for the derivative $s'(x)$: of $s$: 
  	$$s'(x) = r(x, s(x))  $$
  \end{remark}
  
  We will now show that, for a power series $s$ over a ring which is an unique factorization domain (for example, a field $K$ or any ring of polynomials or formal power series over a field), being algebraic and being part of an algebraic family are the same:
%  \begin{lemma}
...
%  \end{lemma}
  \begin{lemma}[Elimination of $n$ variables in $n$ equations]
  ...
  \end{lemma}
  \begin{proof}
  	This result, based on elimination theory, is proved constructively in (cite van der Waerden, 2, par. 82). In (cite Kuich), it is proved carefully that the annihilating polynomial of each $s_i$  can be computed effectively from the algebraic system.
  \end{proof}
