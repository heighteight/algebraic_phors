...
also its derivative $s'\in \fps{\R}{x}$ is algebraic. Moreover, one can find effectively (\cite{CHUDNOVSKY1986271}, \cite{DBLP:series/tmsc/KauersP11}) a rational function $r(x,y)$ such that $s'(x)$ can be computed from $s$ via 
 $s'(x) = r(x, s(x)) $.
  \end{remark}
	...
	The coefficients $a_n$ of an algebraic fps
$a(x)=\sum_n a_n x^n$ with annihilating polynomial $p(w, x)$ satisfying $p(0,0)\neq 0$ and $\partial_xp\neq 0$ can be computed explicitly via a ``multinomial formula''   \cite{BANDERIER_DRMOTA_2015}.
