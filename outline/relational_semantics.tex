...
For every PHORS $\gphors=(\nonterm, \C R,S)$, we define the interpretation of each non-terminal $L_i\in\nonterm$ as $
\model{L_i}^{\Rsemiring}:=  \model{\pi_i(Yt_{\gphors})}^{\Rsemiring}=\pi_i\left(\fix \model{t_{\gphors}}^{\Rsemiring}\right)\in \Rsemiring^{\model{\nonterm(L_i)}}$.  
In particular, we define:

\begin{definition}
Given a PHORS $\gphors=(\nonterm,\C R,S)$, its \emph{probabilistic generating function}
is the fps $a_{\gphors}(z):=\model{S}^{\fps{\Rinf}{z}}\in \fps{\Rinf}{z}$.
\end{definition}
This formal power series precisely captures the call-by-value probabilistic execution of closed terms, as stated below: 
