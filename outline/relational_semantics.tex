% !TEX root = main.tex
In this section, we recall the well-studied relational semantics of linear logic weighted over a continuous semiring $\Rsemiring$, although in a slightly different flavor than the one used in (cite Laird, Manzonetto): rather than using $\Rsemiring$-valued matrices, we will use formal power series over $\Rsemiring$, as in (cite Paolo and Davide). We will notably illustrate how this semantics yields an interpretation of PHORS as formal power series.
...
Let us first introduce formal power series; for more details about them and their applications to combinatorics we refer to (cite the concrete tetrahedron). Given a set $\Sigma$, 
...
A formal power series with finite support is simply called a \emph{polynomial}. 
As usual, polynomials can be written as finite sums, and 
%  $$p= \sum_{n_1 \dots n_{|\Sigma|} = 0}^k p_{n_1 \dots n_{|\Sigma|}} x_{\sigma_1}^{n_1} \dots x_{\sigma_{|\Sigma|}}^{n_{|\Sigma|}}$$
...
%  \end{lemma}
  ...
  \subsection{The Weighted Relational Semantics}
    ...
    Now, we recall the link between formal power series and the weighted relational model of linear logic (cite Laird, Manzonetto). Given a continous semiring $\Rsemiring$, the category 
...
In order to interpret $\PLY$, we will restrict our attention to semirings of the form $\Rsemiring=\fps{\Rinf}{\{z\}+\Sigma}$, where $z$ denotes a distinguished variable. In this setting we interpret probabilistic choice (with bias $p$) as 
\[
...
The generating function $a_\gphors(z)$ precisely captures the call-by-value probabilistic execution of closed terms, as stated below: 
...
This leads to the following result, stating that the \emph{derivative} of $a_\gphors(z)$ captures the expected number of steps to termination:
