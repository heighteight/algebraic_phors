...
For any $\PLY$ closed term $t:\1$, 
\begin{align}
\model{t}^{\Rinf}&=\mathbb P(t\downarrow),
%=\sum_{R:M\to^* e}w(R),
\label{eq:prob1} \\
\model{t}^{\fps{\Rinf}{z}}(z)&=\sum_{i=0}^{\infty}\mathbb P(t\downarrow_i)z^i,\label{eq:prob2}
\end{align}
where $\mathbb P(t\downarrow_i)=\sum_{R:t\to^i e}w(R)$ is the probability that $M$ terminates after exactly $i$ probabilistic steps.
\end{proposition}
...
\left(\model{t}^{\fps{\Rinf}{z}}\right)'(1)=\sum_{i=1}^{\infty} i\cdot \mathbb P(t\downarrow_i)=\mathbb E(t\downarrow).
\end{equation}
