% !TEX root = main.tex
In this section, we recall the well-studied relational semantics of linear logic weighted over a continuous semiring $\Rsemiring$, although in a slightly different flavor than the one used in \cite{DBLP:conf/lics/LairdMMP13}: rather than using $\Rsemiring$-valued matrices, we will use formal power series over $\Rsemiring$, as in \cite{DBLP:journals/corr/abs-2501-15637}. We will notably illustrate how this semantics yields an interpretation of PHORS as formal power series.
...
let us call $!\Sigma$ the set of finite multisets over it, i.e functions $\mu: \Sigma \to \NAT$ with finite support. Given a semiring $\Rsemiring$, the set of \emph{formal power series (with commuting variables) over $\Rsemiring$}, denoted $\fps{\Rsemiring}{\Sigma}$, is the set of all functions $!\Sigma \to \Rsemiring$. More concretely, if
 we introduce for each $s \in \Sigma$ a variable $x_s$ (we will denote by $x_\Sigma$ the set of all these variables), then a finite multiset $\mu \in !\Sigma$ can be seen as the monomial $x_\Sigma^\mu \bydef \Pi_{s \in \Sigma}x_s^{\mu(s)}$; then any formal power series $s\in \fps{\Rsemiring}{\Sigma}$ can be expressed as a formal sum:
...
A \emph{polynomial} is a fps  with finite support  and can written, as usual, as a finite sum.
They%  $$p= \sum_{n_1 \dots n_{|\Sigma|} = 0}^k p_{n_1 \dots n_{|\Sigma|}} x_{\sigma_1}^{n_1} \dots x_{\sigma_{|\Sigma|}}^{n_{|\Sigma|}}$$
form a (non-continuous) sub-semiring of $\fps{\Rsemiring}{\Sigma}$, that we will denote as $\fpp{\Rsemiring}{\Sigma}$.
...
%  \end{lemma}
If we do not start, as we do here, with a continuous semiring $\Rsemiring$, but rather with a ring $R$, almost all constructions still work: we can similarly define the set $\fps{R}{\Sigma}$ of formal power series over $R$, which is a ring, as well as its subring  $\fps{R}{\Sigma}$ of polynomials over $R$. Still, the composition of formal power series will not be defined in general, as it involves an infinite sum, while the composition of polynomials remains well-defined.
  ...
  \subsection{The Weighted Relational Semantics}
    ...
    Now, we recall the link between formal power series and the weighted relational model of linear logic \cite{DBLP:conf/lics/LairdMMP13}. Given a continous semiring $\Rsemiring$, the category 
     ...
     At the same time, $\Qrelkleisli{\Rsemiring}$ can be seen as a category of formal power series:    since a matrix $(t_{\mu, y})_{\mu \in !X, y \in Y}$ can be seen as a $Y$-indexed family of power series $(\sum t_{\mu, y} x_X^\mu)_{y \in Y}$, the morphisms of $\Qrelkleisli{\Rsemiring}$ can be identified with (families of) formal power series, i.e.~$\Qrelkleisli{\Rsemiring}(X,Y)\equiv\fps{\Rsemiring}{X}^Y$.
...
This allows us to interpret simple types as $\model o=1:=\{\star\}$ and $\model{T\to U}=!\model{T}\times \model{U}$. Intuitively, a type $T$ translates into a set of variables, and a term $\Gamma\vdash t:T$ into a $\model{T}$-indexed family of fps $\model{t}^{\Rsemiring}_i(x_\Gamma)$ in \emph{as many variables as $\model{\Gamma}$}.
Here we can observe the main challenge appearing with higher-order types, as these are interpreted by \emph{infinitely many} variables $\model{T\to U}=!\model{T}\times \model{U}$. For instance, $\model{o\to o}=!\model o\times \model o\equiv\N\times 1\equiv \N$ translates into a countable sequence of variables, and 
...
Consider the program $t=\lambda x.y(yx)$, where $y:(\1\to\1)\vdash t:\1\to\1$: letting $\model{\1}=1=\{\star\}$, and 
...
%\end{align*}
Notice that the evaluation $y:(\1\to\1),x:\1\vdash tx:\1$ is precisely interpreted by $a(a(x))\in \Qrelkleisli{\Rsemiring}(\N+1,1)=\fps{\Rsemiring}{\N+1}\equiv\fps{(\fps{\Rsemiring}{\N})}{1}$.
...
Beyond exponentials, 
$\Qrelkleisli{\Rsemiring}$ is endowed with Conway fixpoints \cite{DBLP:journals/mscs/Hasegawa09}: given a morphism $f \in \Qrelkleisli{\Rsemiring}(X + Y, X)$,  we define $\fix f \in \Qrelkleisli{\Rsemiring}(Y, X)$ as follows: take the sequence $f_0 \bydef  0 \times id_Y \in \Qrelkleisli{\Rsemiring}(1 \times Y ,X \times Y), \: f_{n+1} \bydef f \circ (f_n \times id_Y) \circ \langle id_\1, \Delta_Y \rangle  \in \Qrelkleisli{\Rsemiring}(1 \times Y, X)$ and finally let $\fix f \bydef \sup_n f_n \in \Qrelkleisli{\Rsemiring}( Y, X)$.
...
Using the cartesian closed structure and the fixpoints, we can interpret any term of $\LY$ in $\Qrelkleisli{\Rsemiring}$: letting types be interpreted by $\model{o}=1:=\{\star\}$, $\model{T\times U}=\model T+\model U$, $\model{T\to U}=!\model{T}\times \model U$, we obtain, for all term $\Gamma\vdash t:T$, a $\model{T}$-indexed family of fps
...
\[
\model{M\oplus_p N}^{\Rsemiring}=p z\cdot\model M+(1-p) z\cdot\model N.
\]
...
For every PHORS $\gphors=(\nonterm, \C R,S)$ and non-terminal $L_i:T_1\to\dots\to T_n\to o\in\nonterm$, letting $\Sigma=\model{T_1}+\dots+\model{T_n}$, we define the fps
\[
a_{L}(z)(x_{\Sigma}):=
 \model{\pi_i(Yt_{\gphors})f_1\dots f_n}^{\fps{\Rinf}{z}}\in \fps{(\fps{\Rinf}{z})}{\Sigma}.
\]  
In particular, the \emph{probabilistic generating function of $\gphors$}
is the fps $a_{\gphors}(z):=a_S(z)\in \fps{\Rinf}{z}$.
\end{definition}
Notice that the $a_{L}(z)(x_\Sigma)=\pi_i\left(\fix \model{t_{\gphors}}^{\fps{\Rinf}{z}}\right)(x_\Sigma)$ is  the minimal solutions of the equational system $(r_x=(t_\gphors)_x(r_x,z))_{x\in \Sigma}$. 

The generating function $a_\gphors(z)$ precisely captures the call-by-value probabilistic execution of closed terms, as stated below:%the interpretation $\model{M}^{\Rinf}$ of a term $M:\1$ of ground type consists in a real number, since $\model{M}^{\Rinf}\in\Qrelkleisli{\Rsemiring}(0,\1)\equiv\fps{\Rsemiring}{0}\equiv\Rsemiring$, which coincides with the probability of termination of $M$, computed as the sum of the weights $w(R)\in \Rinf$ of all call-by-name probabilistic reductions $R:M{\to^*}e$:
...
\end{proposition}
\begin{proof}
From \cite{DBLP:conf/lics/LairdMMP13, DBLP:journals/jacm/EhrhardPT18,DBLP:journals/corr/abs-2501-15637} we know that each reduction $S\redbigp{\sigma,p}e$ precisely adds up one monomial $pz^{|\sigma|}$, so the monomial $a_{|\sigma|}z^{|\sigma|}$ in $a_\gphors(z)$ is the sum of the probabilities of all reductions $S\redbigp{\sigma,p}e$.
\end{proof}
...
From Proposition \ref{prop:proba} we deduce also that the \emph{derivative} of $a_\gphors(z)$ captures the expected number of steps to termination:
...
$s^F_i(y_{\N+1})\in \fps{(\fps{\Rinf}{z})}{\N +1}$, given, for $i\in \N$, and recalling $s_i(y_{\N})$ from Example \ref{ex:churchtwo}, by
\[
s^F_i(y_{\N+1})=
\begin{cases}
\frac{1}{2}zs_1(y_{\N})+\frac{1}{2}z=\frac{1}{2}(z
y_1^2+z)
& \text{ if }i=1,\\
\frac{1}{2}zs_i(y_{\N})
&\text{ if }i\in\N, i\neq 1,
...
Computing $\model{t_{\gphors}}^{\Rinf}= \mathsf{fix}\  \langle s^F,s^S \rangle$ means finding a minimal solution $(a_i(z))_{i\in \N+1}\in(\fps{\Rinf}{z})^{\N+1}$ of the fixpoint equations $a_{i}(z)=s^F_i(a_{\N+1(z)})$ ($i\in\N$) and $a_{\gphors}(z)=s^S(a_{\N +1}(z))$.
This yields then $a_{i}(z)=0$, for $i\neq 1,\star$, while $a_\gphors(z):=a_\star(z)=a_1(z)\in \Rinf$ is a minimal solution of
...
We obtain then that $\mathbb P[\gphors\downarrow]=a_{\gphors}(1)=1$, that is, $\gphors$ is AST, and 
 that $\mathbb E[S\downarrow]$ is given by the diverging series $a'_{\gphors}(1)=\sum_{i=0}^{\infty}\frac{2i+1}{2^{2i+1}}C_i=
