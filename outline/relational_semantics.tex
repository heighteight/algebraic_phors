% !TEX root = main.tex
In this section, we recall the well-studied relational semantics of linear logic weighted over a continuous semiring $\Rsemiring$, although in a slightly different flavor than the one used in (cite Laird, Manzonetto): rather than using $\Rsemiring$-valued matrices, we will use formal power series over $\Rsemiring$, as in (cite Paolo and Davide's work about tropical lambda calculus). We will notably illustrate how this semantics yields an interpretation of PHORS as formal power series.
...
For every PHORS $\gphors=(\nonterm, \C R,S)$, we can define the interpretation of each non-terminal $L_i$ as 
\[
\model{L_i}^{\Rsemiring}:=  \model{\pi_i(Yt_{\gphors})}=\pi_i\left(\fix \model{t_{\gphors}}\right)\in \Rsemiring^{\model{\nonterm(L_i)}}.
\]  
When $\Rsemiring=\Rinf$, the interpretation precisely captures the call-by-value probabilistic execution of closed terms, as stated below: 
...
%=\sum_{R:M\to^* e}w(R),
\label{eq:prob1} \\
\model{S}^{\fps{\Rinf}{z}}(z)&=\sum_{i=0}^{\infty}\mathbb P(\gphors\downarrow_i)z^i,\label{eq:prob2}
\end{align}
where $\mathbb P(\gphors\downarrow_i)=\sum_{R:t\to^i e}w(R)$ is the probability that $S$ terminates after exactly $i$ probabilistic steps.
\end{proposition}
...
\left(\model{S}^{\fps{\Rinf}{z}}\right)'(1)=\sum_{i=1}^{\infty} i\cdot \mathbb P(\gphors\downarrow_i)=\mathbb E(\gphors\downarrow).
\end{equation}
...
$s^F_i(y_{\N+1})\in \fps{\Rinf}{\N +1}$, given, for $i\in \N$, and recalling $s_i(y_{\N})$ from Example \ref{ex:churchtwo}, by
\[
s^F_i(y_{\N+1})=
\begin{cases}
...
Computing $\model{t_{\gphors}}^{\Rinf}= \mathsf{fix}\  \langle s^F,s^S \rangle$ means finding a minimal solution $(a_i)_{i\in \N+1}\in(\Rinf)^{\N+1}$ of the fixpoint equations $a_{i}=s^F_i(a_{\N+1})$ ($i\in\N$) and $a_{\star}=s^S(a_{\N +1})$, and thus in particular, a minimal solution $a_1\in \Rinf$ of
