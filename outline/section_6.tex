...
The interpretation of $L$ will be some power series $a_L(f,g,x)=\sum_{ij}L_{ij}f^ig^jx$.
Now, observe that $a_L(f,g,x)$ can be rewritten as a power series $a_L(f,g)(x)$ with coefficients in $\fps{\Rinf}{f,g}$, captured (letting e.g.~$a=\frac{1}{2}$) by the algebraic equation  
\[
a_L(f,g)=\frac{f a_L(f,g)^2+g}{2}.
\] 
This equation is reminiscent of \eqref{eq:alg1} and can indeed be solved in a similar way, yielding $a_L(f,g,x)=\sum_{i=0}^{\infty}\frac{C_i}{2^{2i+1}}f^ig^{i+1}x$.
