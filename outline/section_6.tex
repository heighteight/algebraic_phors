...
\begin{align*}
L_{[A,B]}x&= f(L_{[A,B]}(L_{[A,B]}x))\oplus_a gx\\
S&= L_{[A,B]}e
\end{align*}
The interpretation of the new order-1 non-terminal $L_{[A,B]}:\tau$ is then the fps $a_{F}(z,f,g)(x)\in \fps{(\fps{\Rinf}{z,f,g})}{x}$ given by Equation \eqref{eq:catalan2}.
\end{example}
...
\end{corollary}
Notice that, if we apply this technique to the $\Ginf$ from Example \ref{ex:phorsLfx},
we precisely get the one in Example \ref{ex:Lfxpara}.
