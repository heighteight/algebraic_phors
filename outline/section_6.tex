...
We now introduce a second class of PHORS that extends the $\Gfin$ by admitting infinite grades $!_\infty$, corresponding to parameters that may be used an \emph{unbounded} number of times. In this way, thanks to the composability of the underlying algebraic power series, the $\Ginf$ are proved to be closed under arbitrary {compositions}.
...
the variable $z$ in $a_\gphors(z)$, which counts the number of probabilistic choices (e.g.~think again of the HORS \eqref{eq:introphors2}), can be thought of as an input that is used \emph{unboundedly} during reductions, yet without compromising algebraicity.
...
$\varphi\sqsubseteq\varphi',\Psi\sqsubseteq\Psi', k\leq h\Rightarrow !_k\varphi\multimap \Psi\sqsubseteq !_h\varphi'\multimap\Psi'$
...
ax$_{\C P}$ and, notably, the infinitary abstraction and application rules $\lambda_\infty$ and $@_\infty$, which ensure that an infinitary function $t:!_\infty\varphi\multimap\Psi$ can only be applied to either a non-terminal or a parameter. 
...
\begin{align*}
			&\begin{array}{ccc}
		\infer[ax_\Delta]{\nonterm\mid\C P\mid \Delta\vdash x:\varphi }{x:_p\varphi\in \Delta,\quad 1 \leq p} &
...
\begin{example}
One can check that all PHORS in (\cite{DBLP:journals/lmcs/KobayashiLG20}, Examples 2.9-2.11) are $\Ginf$, but not $\Gfin$, e.g.~the (non-AST) tree-generation program below:
\begin{align*}
S&= \mathrm{Treegen} \ \mathrm{Boolgen} \ e,\\
\mathrm{Boolgen}\ x&= x,\\
\mathrm{Treegen} \ f\  x &= x\oplus_{\frac{1}{2}}f(\mathrm{Treegen}\ f( \mathrm{Treegen} \ f( \mathrm{Treegen}\ f\ x))).
\end{align*}
The non-terminal $\mathrm{Treegen}$ uses $f$ unboundedly, but the same parameter $f$ occurs as first argument in all instances of $\mathrm{Treegen}$, which can thus be given type $!_\infty\tau \multimap \tau$.
 \end{example}


As for the $\Gfin$, we interpret each $\Ginf$ $\gphors=(\nonterm,\C P, \C D,S)$ via its
induced simply typed PHORS $(\sat\nonterm,\sat{\C P},\sat{\C D},S)$.
This yields then a family of fps
${\model{{\gphors}}}^{\Rsemiring}\in \fps{\Rsemiring}{\model{\sat\nonterm}+\model{\sat{\C P}}}^{\model{\sat{\nonterm}}}$, that we will look at as parametric on $\C P$, that is, as 
in $\fps{(\fps{\Rsemiring}{\model{\sat{\C P}}})}{\model{\sat\nonterm} }^{\model{\sat\nonterm}}$.
%
...
First, we will consider finitary PHORS with parameters, which can be shown algebraic with the same approach as in Section 6. Then, we show that any $\Ginf$ can be transformed into an equivalent parametric $\Gfin$, which will result in the algebraicity of its generating function: the fundamental intuition behind this procedure is that the variables $\vec w$ of infinite grade can be in the end ``discharged'' onto the coefficient semiring, which is turned from $\fps{\Rinf}{z}$ to $\fps{\Rinf}{z,\vec w}$.


\subparagraph*{Finitary Parametric PHORS}
...
By arguing as in the previous section, but reasoning over the semiring $\fps{\Rsemiring}{\model P}$, we can conclude that the pair of finite variable sets $(\model \nonterm,\model \nonterm)$ is stable and polynomial, yielding:

\begin{proposition}
For all finitary parametric $\Gfin$ $\gphors=(\nonterm, \C P,\C D, S)$, the fps $a_L(z,w_{\model{\C P}})\in \fps{\Rinf}{z,w_{\model{\C P}}}$ are all algebraic. 
\end{proposition}

\begin{example}\label{ex:Lfxpara}
Consider the $\Gfin$ with parameters $f,g:_\infty\tau$ obtained from the one in Example \ref{ex:phorsLfx} by adding two new non-terminals
 $L_{[f,g]}:\tau$ and $S_{[f,g]}$ with equations 
\begin{align*}
L_{[f,g]}x&= f(L_{[f,g]}(L_{[f,g]}x))\oplus_a gx\\
S_{[f,g]}&= L_{[f,g]}e
\end{align*}
The interpretation of $L_{[f,g]}:\tau$ is the fps $a_{F}(z,f,g)(x)\in \fps{(\fps{\Rinf}{z,f,g})}{x}$ given by \eqref{eq:catalan2}.
\end{example}


\subparagraph*{Reducing $\Ginf$ to Parametric $\Gfin$}
...
let $\C P_L$ be formed by all $w_i:_\infty\phi_i$, and let $T(L) \bydef \{[t_1, \dots t_n] \in (\nonterm\cup\C P)^* \: \mid \: \forall i \, (\nonterm \cup \mathcal{P}) (t_i) = \phi_i)  \}$ be the sets of \emph{sostituable terms}.
...
For each $L\in \nonterm$, $\gamma\in T(L)$,
and term $\nonterm\mid \vec v:\C P_L\mid\vec x:\Delta\vdash t_L:o$,
% let $t_{L,\gamma}:= \lam \vec x.t_L[\gamma/\vec w]: \phi_L$ and 
 define:
\begin{varitemize}
\item an applicative term $\nonterm\mid\C P\mid\emptyset \vdash \alpha_{L,\gamma}:\phi_L$ defined by 
$\alpha_{L,\gamma}:=L\gamma$;

\item a finitary term $\widetilde{\nonterm}\mid\vec w:\C P \mid\vec x:\Delta\vdash \tilde{t}_{L,\gamma}:o$ defined by 
$\tilde t_{L,\gamma}:=  t_{L}[\gamma/\vec v]\big[ \tilde G_{\delta}/G\delta\big]_{G\in \nonterm, \delta\in T(G)}$.
Notice that $\lambda \vec x.\tilde t_{L,\gamma}:\phi_L$.
\end{varitemize}


We have obtained in this way a finitary parametric PHORS $\gphors_{\mathrm{fin}}=(\widetilde\nonterm, \C P, \widetilde{\C D},S_{[]})$,
whose interpretation is a FAS with coefficients in $\fps{\Rinf}{z,\vec w}$. 
By inspecting the connected component formed by all non-terminals $\tilde L_{\gamma}$
``reachable'' from the source $S_{[]}$, one can check that none of these contains free parameters in $\gamma$. 
This implies in particular that the generating function $a_{\gphors_{\mathrm{fin}}}(z,\vec w)=a_{S_{[]}}(z,\vec w)$ does not actually depend on the parameters $\vec w$. We will show that $a_{\gphors_{\mathrm{fin}}}(z,\vec w)$ coincides with $a_\gphors(z)$.
%Now let $\widetilde{\nonterm}_0$ be the restriction of $\widetilde{\nonterm}$ to those non-terminals $\tilde{L}_\gamma$ that are reachable starting from $S_{[]}$. One can easily see that for all such $\tilde{L}_\gamma$, the list $\gamma$ contains no parameters. By restricting to $\widetilde{\nonterm}_0$ we obtain then a \emph{closed} 
%$\Gfin$ $\gphors_0^{\mathrm{fin}}=(\widetilde\nonterm_0, \C D'_0,S_{[]})$, whose interpretation is now a FAS with coefficients in $\fps{\Rinf}{z}$. 
Observe that $\gphors$ and $\gphors_{\mathrm{fin}}$ are related by the following:
\begin{equation}\label{eq:alphaeq}
\nonterm\mid\C P\mid\emptyset\vdash \ \alpha_{L,\gamma}\big[ \lambda \vec v.\lambda \vec x.t_L/  L] \RED{}{} \lambda \vec x.t_{L}[\gamma/\vec v]   =\lambda \vec x.
\tilde{t}_{L,\gamma}\big[\alpha_{G,\delta}/\tilde{G}_\delta\big]_{G\in \nonterm,\delta\in T(G)} 
\ : \ \widetilde{\nonterm}(L)
\end{equation}


The lemma below relates the minimal solutions of the systems of $\gphors$ and $\gphors_{\mathrm{fin}}$:
...
\end{varenumerate} 
then
$\alpha\circ \langle {\mathsf{fix}}_{0,\nonterm} t,1_{\C P}\rangle={\mathsf{fix}}_{\C P,\widetilde{\nonterm}} \tilde t\in \Qrelkleisli{\Rsemiring}(\C P, \widetilde{\nonterm})\equiv \Qrelkleisli{\fps{\Rsemiring}{\C P}}(1,\widetilde{\nonterm})
$. 

\end{lemma}
%
%\begin{proposition}
%$a_{\gphors}(z)=a_{\gphors^{\mathrm{fin}}}(z)$.
%\end{proposition}
%\begin{proof}[Proof sketch]
%Condition 1.~of Lemma \ref{lemma:fixpoint} clearly holds for the interpretation of $\alpha=\langle \model{\alpha_{L,\gamma}}^{\Rsemiring}\rangle_{L,\gamma}$, and condition 2.~is precisely \eqref{eq:alphaeq}, hence $\fix \model{t_{\gphors^{\mathrm{fin}}}}= \alpha\circ \langle \fix\model{t_{\gphors}},1_{\C P}\rangle$.
%Since $\pi_{S_{[]}}\circ \alpha= \alpha_{S,[]}$ and 
%$\alpha_{S,[]}(\langle L_1,\dots, L_n\rangle,\vec w)= S$, i.e.~$\alpha_{S,[]}=\pi_S\circ \pi_2$, we have 
%$
%a_{\gphors^{\mathrm{fin}}}(z)=
%\pi_{S_{[]}}\circ \fix \model{t_{\gphors^{\mathrm{fin}}}}=
%\pi_{S_{[]}}\circ \alpha\circ \langle \fix\model{t_{\gphors}},1_{\C P}\rangle=
%\alpha_{S,[]}\circ \langle \fix\model{t_{\gphors}},1_{\C P}\rangle= 
%\pi_S\circ \fix\model{t_{\gphors}}= a_{\gphors}(z)$.
%\end{proof}

\begin{proposition}
	\label{theorem:infalgebraic}
For any non-terminal $L\in \nonterm$, $a_L(z,\vec w_L,\vec x)= a_{\tilde{L}_{\vec w_L}}(z,\vec w,\vec x)$, where 
the $\vec w_L$ are the variables $w$s belonging to $\model{\C P_L}$, 
%In particular, 	
%$a_{\gphors}(z)=a_S(z)=a_{S_{[]}}(z,1)=a_{\gphors_{\mathrm{fin}}}(z,1)$.
and $a_L(z,\vec w_L,\vec x)$ is thus algebraic.


\end{proposition}
\begin{proof}[Proof sketch]
Condition 1.~of Lemma \ref{lemma:fixpoint} clearly holds for the interpretation of $\alpha=\langle \model{\alpha_{L,\gamma}}^{\Rsemiring}\rangle_{L,\gamma}$, and condition 2.~is precisely \eqref{eq:alphaeq}, hence $\fix \model{t_{\gphors^{\mathrm{fin}}}}= \alpha\circ \langle \fix\model{t_{\gphors}},1_{\C P}\rangle$.
Since $\pi_{\tilde{L},\vec w_L}\circ \alpha= \alpha_{L,\vec w_L}$ and 
$\alpha_{L,\vec w_L}(\langle L_1,\dots, L_n\rangle,\vec w)= L(\vec w_L)$, we have 
$
a_{\gphors_{\mathrm{fin}}}(z,\vec w,\vec x)=
\pi_{L,\vec w_L}\circ \fix \model{t_{\gphors_{\mathrm{fin}}}}=
\pi_{L,\vec w_L}\circ \alpha\circ \langle \fix\model{t_{\gphors}},1_{\C P}\rangle=
\alpha_{L,\vec w_L}\circ \langle \fix\model{t_{\gphors}},1_{\C P}\rangle= 
 a_{L}(z,\vec w_L,x)$.
%
%
%Since $\pi_{S_{[]}}\circ \alpha= \alpha_{S,[]}$ and 
%$\alpha_{S,[]}(\langle L_1,\dots, L_n\rangle,\vec w)= S$, i.e.~$\alpha_{S,[]}=\pi_S\circ \pi_1$, we have 
%$
%a_{\gphors^{\mathrm{fin}}}(z,\vec w)=
%\pi_{S_{[]}}\circ \fix \model{t_{\gphors_{\mathrm{fin}}}}=
%\pi_{S_{[]}}\circ \alpha\circ \langle \fix\model{t_{\gphors}},1_{\C P}\rangle=
%\alpha_{S,[]}\circ \langle \fix\model{t_{\gphors}},1_{\C P}\rangle= 
%\pi_S\circ \fix\model{t_{\gphors}}= a_{\gphors}(z)$.
\end{proof}
...
%)
%
%and $\C L(\gphors)=\C L(\gphors_{\mathrm{fin}})$.
%
%	
%For any closed $\Ginf$ $\gphors$ there exists a parametric $\Gfin$ $\gphors_{\mathrm{fin}}$ of size $\|\gphors_{\mathrm{fin}}\|\leq \|\gphors\|^{|\nonterm|}$ such that $a_{\gphors}(z)=a_{\gphors_{\mathrm{fin}}}(z,1)$ and $\C L(\gphors)=\C L(\gphors_{\mathrm{fin}})$.
%
We thus immediately obtain:
\begin{corollary}
For any closed $\Ginf$ $\gphors$ there exists a parametric $\Gfin$ $\gphors_{\mathrm{fin}}$ of size $\|\gphors_{\mathrm{fin}}\|\leq \|\gphors\|^{|\nonterm|}$ such that $a_{\gphors}(z)=a_{\gphors_{\mathrm{fin}}}(z,1)$ and $\C L(\gphors)=\C L(\gphors_{\mathrm{fin}})$.
In particular, the AST and PAST problems for $\gphors$ are decidable.
\end{corollary}
\begin{example}
