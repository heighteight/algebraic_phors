...
Now, in the relational interpretation, each order 2 non-terminal symbol $N$ yields a power series $\model{N}(y,x)$, and thus a sequence of coefficients $N_n$, and each order 1 non-terminal $N$ yields a unique real coefficient. 
...
We can also obtain a first-order formula expressing PAST via the following two observations.
...
C(w)= \frac{wC^2(w)+w}{2},
$$
whose solution gives precisely the generating function of the \emph{Catalan numbers}
...
with coefficients taken themselves from a semiring of formal power series. In the relational semantics, this corresponds to lifting the underlying continuous semiring of weights from the (extended) positive reals $\BB R_{\geq 0}^\infty$ to the semiring of formal power series
