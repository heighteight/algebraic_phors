...
Behind the difficulty of estimating the probability of termination of higher-order programs lies the challenge of \emph{counting} probabilities over a typically \emph{infinite} set of reductions, even when these reductions follow some well-defined recursive pattern. 
...
The generating function $c(x)=\sum_{n=0}^{\infty}C_n x^n$ can be characterized as the solution of a simple algebraic equation, namely $xc(x)^2-c(x)+1=0$. 
Such algebraic descriptions can be used to capture several properties of the corresponding sequences (and, in some cases, even to provide an explicit formula for all of them \cite{BANDERIER_DRMOTA_2015}). 
In the case above, it easily leads to a closed expression $c(x)=\frac{1-\sqrt{1-4x}}{2x}$ that can be used to compute $c(x)$ effectively.
