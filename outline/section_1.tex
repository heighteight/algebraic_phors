...
A fundamental reason underlying the difficulty of estimating the probability of termination of higher-order programs is that this requires to \emph{count} probabilities over a typically infinite set of reductions, even though such reductions might all follow a somehow finitary pattern. 

Such counting problems are common in the field of combinatorics, in which they are addressed by studying the algebraic and analytic properties of the generating function associated with the sequence of numbers that one wishes to compute or estimate.   
...
Since Chomsky and Sch\"utzenberger's pioneering work (cite), generating functions have been widely applied in formal language theory. For example, it is well-known that for all regular languages $L$, the associated generating function $L(x)=\sum_{n=0 }^{\infty}L_nx^n$, where $L_n$ counts the words of $L$ of length $n$, is \emph{rational}, that is, it can be rewritten as a fraction $L(x)=p(x)/q(x)$ of two polynomials.
%
%
...
When $L$ is context-free, instead, the generating function $L(x)$ is \emph{algebraic}, that is, is the solution, like $c(x)$ above, of some polynomial equation $p(x,L(x))=0$. 
...
Now, suppose $t$ is some program with the ability of performing, at each of its reduction steps, an unbiased choice between two different alternatives. 
Letting $T_n$ be the number of \emph{distinct} terminating reductions of $t$ with exactly $n$ steps, the generating function $a_t(x)=\sum_{n=0}^{\infty}\frac{T_n}{2^n}x^n$ captures the probabilistic behavior of $t$. Notably, $a(1)$ precisely computes the probability that $t$ terminates, and, as we'll see, the derivative $a'(1)$ precisely captures the \emph{expected} number of reduction steps to termination. 
...
\paragraph*{The Weighted Relational Model}

While such combinatorial approaches in the literature are focused on the study of \emph{first-order} programming languages (also in the probabilistic case, cite people), the main goal of this work is to show that the this natural and powerful idea can be applied to study the termination problem for probabilistic \emph{higher-order} recursive programs, like the PHORS.

%
%Combinatorial approaches to probabilistic termination have  applied in several works (cite works) for the study of \emph{first-order} probabilistic languages.
%The goal of this work is to demonstrate how it can be applied to study the termination problem for \emph{higher-order} languages as well, and notably to the PHORS language.
%
...
The first fundamental ingredient is the \emph{weighted relational model} (cite Laird Manzonetto etc.), which associates proofs in linear logic, as well as PCF programs, with families of \emph{formal power series} with coefficients in a continuous semiring.
We show how this model can be used to associate PHORS with generating functions capturing their probability of termination.


As mentioned before, linearity (or, better, affinity) has been recognized as a key factor to ensure the decidability of the AST problem for PHORS (cite Ong).
A key observation is then that, while affine PHORS can express languages which are \emph{not} context-free, the relational model associates such PHORS with generating functions that, as for the context-free languages, are always algebraic. Actually, this property remains true even if we go beyond linearity/affinity and consider \emph{non-linear} PHORS typable via the discipline of \emph{graded} or \emph{bounded exponentials} (cite): this discipline allows one to define types of the form $!_n A \multimap B$, intuitively expressing functions from $A$ to $B$ using their inputs \emph{at most} $n$ times.

Hence, on the one hand, our approach provides a \emph{new} method, grounded on algebraic combinatorics, to understand (and to re-establish) the decidability of AST/PAST for affine PHORS; on the other hand, it leads us to capture a \emph{larger} class of PHORS, including PHORS admitting both bounded and unbounded forms of duplication, for which these problems remain decidable.
...
\item Then, we introduce the \emph{finitely bounded PHORS}, $\Gfin$, extending the affine PHORS of (Ong) with bounded non-linearity, and show that the corresponding generating functions are algebraic, leading to the decidability of AST and PAST.
