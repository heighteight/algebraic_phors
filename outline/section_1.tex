...
Despite the aforementioned results, the nature of the problem remains poorly understood, and a precise assessment of its computational difficulty is still lacking. For instance, it is currently unknown whether the decidability problem for PHORS remains $\Pi^0_2$-complete, or whether it can instead be placed lower in the arithmetical hierarchy while still being undecidable. Similarly, the fragments of the PHORS language for which decidability results are known are incomparable, and no class of PHORS encompassing all of them has been identified. We therefore believe that it is important to investigate the nature of this problem in greater depth, which is the goal of this paper. In the remainder of this introduction, we present a new perspective that will allow us to shed fresh light on the problem.
