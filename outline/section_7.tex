...
The connections between the semantics of linear logic and combinatorics/formal power series can be traced back to Girard's
quantitative semantics \cite{GIRARD1988129} (and Lamarche's infinitary algebras \cite{LAMARCHE199237}) and their connections with Joyal's theory of analytic functors and combinatorial species \cite{JOYAL19811}. 
As in \cite{DBLP:journals/corr/abs-2501-15637}, we here take at face value the correspondence between morphisms in $\Qrelkleisli{\Rsemiring}$ and formal power series. Similar ideas were already exploited in \cite{DBLP:journals/jacm/EhrhardPT18} to establish the full abstraction of the related semantics of probabilistic coherent spaces.

%
% in the related semantics of probabilistic coherent spaces (PCOH) \cite{DBLP:journals/jacm/EhrhardPT18} correspond to power series; the idea of interpreting probabilistic programs using parameters, i.e.~as \emph{formal} power series, is used in \cite{DBLP:journals/jacm/EhrhardPT18} for PCOH, and is explicitly developed for $\Qrelkleisli{\Rsemiring}$ in \cite{DBLP:journals/corr/abs-2501-15637}. 
% 
...
In the setting of imperative programming, the idea of using generating functions to analyse probabilistic programs is developed in \cite{Klinkenberg2021} and \cite{Klinkenberg2024}; 
...
In \cite{DBLP:journals/lmcs/KobayashiLG20}, the problem of AST for PHORS is studied by directly translating a PHORS into (possibly higher-order) real functional equations: only in the case of order 1 PHORS, this produces a fixpoint algebraic system analogous the those considered in this work, thus implying a decidability result (while in the same work, undecidability since order-2 is proved). Techniques to approximate from above the probability of termination are also considered. In \cite{DBLP:conf/lics/LiMO22}, the decidability of AST/PAST for affine PHORS is proved. With respect to our approach, the translation from PHORS to real equations is defined via an intermediate automata-theoretic step, based on the equivalence between affine HORS and restricted pushdown automata proved in \cite{DBLP:conf/mfcs/ClairambaultM19}.



\section{Conclusion}

In this work we have shown that the combinatorial method of generating functions can be adapted, via the weighted relational semantics of linear logic, to the study of higher-order \emph{probabilistic} languages. 
We think that the main value of this work resides in opening the possibility of applying well-established methods from algebraic and analytic combinatorics to higher-order languages. We provided a first demonstration of this by showing how  the decidability of termination for certain classes of PHORS (proved in the literature with syntactic methods like automata theory or game semantics) can be re-established, and actually \emph{extended}, in a relatively elementary way, via the notion of algebraic power series.
...
And, more broadly, could all these methods help shed light on 
%We also hope that these combinatorial methods might help in settling 
the open problem of exactly placing the AST problem for PHORS within the arithmetic hierarchy?
%
