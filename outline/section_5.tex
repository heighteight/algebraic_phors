...
In this section we introduce the class $\Gfin$, extending the affine PHORS of (Li et al.), whose type discipline ensures that the corresponding generating functions are algebraic. We will show that both the AST and PAST problems are decidable for the PHORS typable in this system.
...
The typing rules will use two kinds of contexts (as in Li et al.): 
\begin{varitemize}
...
\begin{theorem}[Linearization of $\Gfin$]
For every $\Gfin$ $\gphors=(\nonterm, \C R, S)$ there exists a PAHORS $\AFF\gphors=(\nonterm, \C R',S)$, with same non-terminals, size $\|\AFF\gphors\|\leq \partial\gphors{\|\gphors\|}$ and such that 
...
The following examples shows that $\|\AFF\gphors\|$ may grow exponentially in the size of $\gphors$:
...
% \end{definition}
   ...
   Define, for every type $\varphi$, $\model{\varphi}$ as $\model{o^n}=1$ and $\model{!_k\varphi\multimap \psi}=!_k\model{\varphi}\times\model{\psi}$. Notice that $\varphi\tri T$ implies $\model{\varphi}\subseteq\model{T}$, but the variable sets $\model{\varphi}$ are always finite. 
   
   Our general idea is the following: given a $\Gfin$ $\gphors=(\nonterm, \C R, S)$ and supposing $\nonterm\tri T$, from the derivations of $\nonterm\mid\emptyset \vdash t_L:\nonterm(L)$, we deduce a corresponding simply typed derivation of $T\vdash t_{\gphors}:T$ yielding a family
   $\model{t}^{\Rsemiring}\in \fps{\Rsemiring}{\model{T}}^{\model{T}}$.  
  At the same time, we will show that the finite set $\model{\nonterm}\subset\model{T}$ is both stable and polynomial, and we will conclude that $\model{t_{\gphors}}$ is finitary, hence, by Corollary \ref{cor:propertoalg}, algebraic.    
...
$\Gamma\vdash t: T$, the pair $(\model{\nonterm}+\model{\Delta}, \model{\varphi})$ is stable and polynomial for $\model{t}^{\Rsemiring}$.
  ...
  Let a \emph{finitary} variable be any point in the interpretation of ${\nonterm},{\Delta},{\varphi}$, and a \emph{non-finitary} variable be any other point in the interpretation of $\Gamma$ and $T$.
   ...
   \item for all finitary variables $\sigma$ and for all $x:_k\psi\in \Delta$, the fps
   $\left(\model{t}^\Rsemiring\right)_\sigma\Big\vert_{\model{\nonterm}+\model{\Delta}}$ has 
   finite degree in each variable $x_{L:\nonterm(L)}$ and 
   degree at most $k$ in each variable $x_{f:_k\psi}$.
   \end{varenumerate}
...
%
%
An immediate consequence of the stability Lemma \ref{lemma:stability} is the following:
%
...
\end{corollary}
\begin{remark}
	By interpreting  $t_L \oplus t_R$ as $x \model{t_L}^{\Rinf} + \overline x \model{t_L}^{\Rinf}$ (cf.~Remark \ref{rem:tropical}), the previous corollary could be strengthened to say that the set
	$\{(n,m) \mid \:  S \text{ terminates with } n \text{ left steps and } m \text{ rigth steps}\}$ is also semilinear.
	This could also been deduced by the fact that the branch language of a linear HORS is multiple context-free (cite  Clairambault) and languages of this class are known to be semilinear (cite CHARACTERIZING STRUCTURAL DESCRIPTIONS PRODUCED BY VARIOUS.
	GRAMMATICAL FORMALISMS), but it is still worth noticing that we reproved this fact via generating functions.
\end{remark}
%\begin{example}
...
Let us conclude by showing how to decide the AST and PAST problems:
\begin{theorem}
...
Once we have the FAS, we can check if $\mathbb P(\gphors\downarrow) = a_\gphors(1)=1$ via the first order Existential Theory of the Reals $\Etor$ (cite Etessami-Yannakakis, 09): first, we create a first order formula $\phi( \vec x)$ expressing the fact that $\vec x$ satisfies $\vec x = p_i(\vec x)$, then we can express the fact that $x_1=1$ by the formula $\exists \vec{x}(x_1=1 \land \phi(\vec x) \land \forall \vec y ((\vec y < \vec x) \implies \neg \phi(\vec y)  ))$\\
As for PAST, remember in the first place that PAST implies AST. Then, given a $\gphors=(\nonterm, \C R, S) \in \Gfin$, we can in the first place test if it is AST: if it is not, then it not PAST. If it AST, notice that by \ref{Gfin-algebraic-corollary}, $a_\gphors(z)$ is an algebraic power series. Then, we can, as pointed out in remark \ref{derivative-rational}, find a rational function $r(z, y)$ such that $a'_\gphors(z)= r(z, a_{\gphors}(z)) $. But then $E(\gphors\downarrow) =a'_\gphors(1)= r(1,1)$.
\end{proof}
%
%
%{\color{red}
