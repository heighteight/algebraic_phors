...
Now, for a term $t:\varphi$ typable in $\Gfin$, where $\varphi\tri T$, we will define its interpretation $\model{t:\varphi\tri T}^{\Rsemiring}$ as a suitable \emph{finite restriction} of the interpretation $\model{t:T}^{\Rsemiring}$ of the corresponding simply typed derivation.
%
...
\begin{theorem}[$\Gfin$ are algebraic]
For all $\Gfin$ $\gphors=(\nonterm, \C R, S)$, $\model{S}^{\Rinf}\in \Rinf$ is $\mathbb Q^{+}$-algebraic and 
 $\model{S}^{\fps{\Rinf}{z}}\in \fps{\Rinf}{z}$ is $\fps{\mathbb Q^{+}}{z}$-algebraic.
