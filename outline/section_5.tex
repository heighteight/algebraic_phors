...
Observe first that the rules (and indeed the derivations) of $\Gfin$ can be seen as \emph{quantitative refinements} of the simply typed ones. Indeed, every type $\varphi$ is (uniquely) a \emph{refinement} of some simple type $\st{\varphi}$: the simple types $\st\varphi$ are defined by $\st{(o^n)}=o^n$ and $\st{(!_k\varphi\multimap\psi)}=\st\varphi\to\st\psi$.
Notice that $\varphi\sqsubseteq \psi$ implies $\st\varphi=\st\psi$. 
Moreover, for every derivation $\pi:\nonterm\mid\Delta \vdash t:\varphi$, 
by replacing each $\psi$ by $\st\psi$, we obtain, by induction, a simply typed derivation $\st\pi:\st\nonterm,\st\Delta\vdash t:\st\varphi$.
...
For every derivation $\nonterm\mid\Delta\vdash t:\varphi$, the pair $(\model{\nonterm}+\model{\Delta}, \model{\varphi})$ is stable and polynomial for $\model{t}^{\Rsemiring}\in \fps{\Rsemiring}{\model{\st\nonterm}+\model{\st\Delta}}^{\model{\st\varphi}}$.
  
  
  
  \end{lemma}
  \begin{proof}[Proof sketch]
  Let a \emph{finitary} variable be any $\sigma\in\model{\nonterm},\model{\Delta},\model{\varphi}$, and a \emph{non-finitary} one be any $\sigma\in\model{\st\nonterm}-\model{\nonterm},\model{\st\Delta}-\model{\Delta},\model{\st\varphi}-\model{\varphi}$.
...
$\model{t_{\gphors}}^{\Rsemiring}\in \fps{\Rsemiring}{\model{\st\nonterm}}^{\model{\st\nonterm}}$ is finitary (via $\model\nonterm$).
...
As for PAST, remember that PAST implies AST. Then, given a $\gphors=(\nonterm, \C D, S) \in \Gfin$, we can first test if it is AST: if it is not, then it is not PAST. If it AST, notice that, by Corollary \ref{Gfin-algebraic-corollary}, $a_\gphors(z)$ is an algebraic power series. Then, we can, as pointed out in Remark \ref{derivative-rational}, effectively find a rational function $r(z, y)$ such that $a'_\gphors(z)= r(z, a_{\gphors}(z)) $. But then $\mathbb E(\gphors\downarrow) =a'_\gphors(1)= r(1,1)$.
\end{proof}
%
%
%{\color{red}
...
%
\begin{remark}
Beyond AST/PAST, whenever $a_{\gphors}(z)=\sum^{\infty}_{i=0} \mathbb P(\gphors\downarrow_i)z^i$ satisfies a polynomial equation of the form
$a_{\gphors}(z)=z\cdot p(a_{\gphors}(z),z)$ (which is the case of all our examples), all its coefficients $\mathbb P(\gphors\downarrow_i)$ are effectively computable (cf.~Remark
\ref{coefficients-rational}, taking $q(w,z)=zp(w,z)-w$).
\end{remark}