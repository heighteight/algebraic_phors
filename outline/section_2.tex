...
If we ignore probabilities, we can consider the \emph{branch language} $\C L(\gphors)$ computed by the PHORS $\gphors$ as follows:
 replace the choice operation $\oplus_{\frac{1}{2}}$ with a binary terminal symbol $c:o\to o\to o$, and consider the following HORS (i.e.~non-probabilistic recursion scheme) $\gphors'$,  where $G:(o\to o\to o)\to (o\to o)$:
...
%\end{aligned}
%\end{equation}
%where $F:(o\to o)\to(o\to o)$, $S:o$, with 
%$o$ a ground type, 
%$(o\to o)=o\multimap o$ the type of \emph{linear} functions from $o$ to itself, that is, of functions that use their input exactly once, $I:(o\to o)$ indicating the identity function and $e:o$ a constant of ground type.
...
In the weighted relational semantics, the non-terminal $H:(o\to o)\to(o\to o)$ yields, as before, a generating function $a_{H}(z,f,x)=\sum_{i}^{\infty}H_{i}(z) f^{i} x$, where
...
%$A,B:(o\to o)$ yield two real values, and $S:o$ yields a real value capturing the probability of termination of the PHORS.
...
In our example, the non-terminal $H$ could be given the bounded type $!_2(o\to o)\multimap (o\to o)$, as $H$ uses its input $f$ twice (indeed, all generating functions $H_i(z)$ are equal to zero, for $i\neq 2$).
...
This will allow us to use also infinitely graded types $!_\infty\sigma\multimap (o\to o)$, although in a restricted way. 
