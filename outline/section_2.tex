...
\right .
\end{equation}
where $F:o\multimap o$. 
This is a very simple example of PHORS: the upper case letters $F,S$ are called \emph{non-terminal} symbols, the execution of the program starts from the \emph{source} non-terminal $S:o$ by applying instances of the equations, read from left to right, as well as probabilistic choices, terminating when the unit constant $e:o$ is, eventually, produced. 
If we ignore probabilities, we can consider the \emph{branch language} $\C L(\gphors)$ computed by the PHORS $\gphors$ as follows:
 replace the choice operation $\oplus_{\frac{1}{2}}$ with a binary variable $c:o\multimap o\multimap o$, and consider the following HORS (i.e.~non-probabilistic recursion scheme) $\gphors'$:
...
\right .
\end{equation}
where now $G:(o\multimap o\multimap o)\to \tau$. 
...
As we said, the fundamental ingredient to extract generating functions from PHORS is the weighted relational model of linear logic. This is sometimes called a \emph{quantitative} semantics, as it provides a \emph{precise count} of the number of times that a program may use each of its inputs during any of its reductions.
In we consider the HORS $\gphors'$, we see that, for any $i\in \N$, the term $Gzx$ has a unique way of terminating using $z$ exactly $i$ times, and yielding $z^{i}x$. This translates into the power series
\[
a_{`Gzx'}(z,x)=\sum_{i>0}^{\infty}z^i x
\]
To get back to the original PHORS $\gphors$, interpret the ``choice'' constant $c:o\multimap o\multimap o$ as the linear map $c(z)(x,y)=\frac{z}{2}x+\frac{z}{2}y$, this yields the generating function
\[
a_{`Fx'}(z,x)=\sum_{i>0}^{\infty}\frac{1}{2^i}z^i x=\frac{x}{2-z}
\]
saying that the probability $Fx$ terminating using $x$ once and $z$ (i.e.~choice) $i$ times is $\frac{1}{2^i}$. 
As $e$ trivially terminates with probability 1, we can associate the source symbol $S$ with the generating function
\[
a_{`S'}(z):=a_{`Fx'}(z,1)=\sum_{i>0}^{\infty}\frac{1}{2^i}z^i=\frac{1}{2-z},
\]
which describes probability of reduction with $i$ choices, and deduce that $\gphors$ terminates with probability $a_{`S'}(1)=1$, i.e. almost surely. 
...
In the weighted relational semantics, the non-terminal $H:\tau\to\tau$ yields, as before, a generating function $a_{`Hfx'}(z,f,x)=\sum_{i}^{\infty}H_{i}(z) f^{i} x$, where
...
$a_{`Hfx'}(z,f,x)= \frac{\beta z}{1-\alpha z^2}f^2x$. 
In this way we can compute 
$a_{`S'}(z)=a_{`Hfx'}(z,1,1)=\frac{\beta z}{1-\alpha z^2}$, and finally deduce the probability of termination $a_{`S'}(1)=\frac{\beta }{1-\alpha }$.
...
Unfortunately, this apparently smooth way of computing termination probabilities cannot be applied in \emph{all} cases: in general, the generating function $a_{\gphors}(z)$ of a PHORS $\gphors$ will be determined by a (countably) infinite family of equations which, in turn, need not be polynomial, which we might not be able to solve. Probability of termination for PHORS is still undecidable, anyway.
...
being, as a consequence, algebraic.
Indeed, by relying, as in (Ong et al.), on the decidable \emph{first order theory of the reals} (cite qualcuno), from the algebraicity of $a_{\gphors}(z)$ we can obtain effective methods to answer both AST and PAST for the corresponding PHORS $\gphors$.


\paragraph*{Algebraic PHORS via Bounded Exponentials}

Our conditions come, once more, from the toolbox of linear logic: 
...
%
%
% family will be in general (countably) \emph{infinite}: 
%already at order 2, as we saw, a function $H:\tau\to \tau$ yields countably many coefficients $(H_n)_{n\in \mathbb N}$, each with its own equation. 
...
A first natural idea is thus to restrict ourselves to linear, or even affine, programs, as in (Ong et al.). Yet, our example shows that one can well admit {bounded} forms of non-linearity. 
...
In our example, the non-terminal $H:\tau \to\tau$ could be also given the bounded type $!_2\tau\multimap \tau$, as $H$ uses its input $f$ twice (indeed, all generating functions $H_i(z)$ are equal to zero, for $i\neq 2$).
%This leads us to introduce the class $\Gfin$ of bounded PHORS. 


But it is possible to can go even beyond that. In our previous example, while we imposed a bound via the type $!_2\tau\multimap\tau$ to the number of uses that $H$ may do of its functional input, we imposed no \emph{a priori} bound on the number of probabilistic choices, i.e.~on the variable $z$.  In other words, if we look at probabilistic choice as some variable $z: o\multimap o\multimap o$, we can let our bounded programs use this variable \emph{as many times} as needed, without compromising algebraicity.
...
The algebraic nature of parametric PHORS leverages the intrinsically parametric nature of the weighted relational semantics, which allows one to pass smoothly from power series $s(x,w)$ in $x,w$, with values over some continuous semiring $\Rsemiring$, to ``parametric'' power series $s(w)(x)$ in $x$ with values themselves of the cointinuous semiring $\fps{\Rsemiring}{w}$ of power series in $x$


Obviously, in order to preserve algebraicity, the use of parameters has to be restricted in some way; still, as a consequence of the composability of algebraic power series, it is possible to \emph{compose} an algebraic PHORS $\gphors_1(w)$, depending on some parameter $w$, with another algebraic PHORS $\gphors_2$, yielding a new PHORS $\gphors_1(\gphors_2)$, in which $\gphors_2$ is used unboundedly. 
...
\paragraph*{Equivalence of Bounded and Affine Branch Languages}

In this work we define two classes of PHORS that extend the affine PHORS of (Ong et al.) by allowing bounded and unbounded duplications, still preserving decidability. A natural question is thus whether such class also extend the class of the corresponding branch languages.
...
In other words, using a rather different, combinatorial, method, we end up characterizing the same class of trees as (Ong et al.), a fact that, in our opinion, suggests the robustness of this decidable class of trees.

This fact will be proved by applying a well-known recipe from linear logic (cite Girard, Mazza,...), which consists in replacing a (bounded) exponential $!_nA$ with an $n$-fold tensor $A\otimes\dots\otimes A$. 
