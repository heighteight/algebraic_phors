...
If we ignore probabilities, we can consider the \emph{branch language} $\C L(\gphors)$ computed by the PHORS $\gphors$ as follows:
 replace each choice $t\oplus_{\frac{1}{2}}u$ with a non-deterministic operation $ct+cu$ marked by an order-1 variable $c:o\to o$; this leads to consider the following (non-deterministic) HORS $\gphors'$,  where $F':( o\to o)\to \tau$:
...
\paragraph*{PHORS with Algebraic Generating Functions}
...
Using distinct constants $a,b,c:o\to o$ for each choice symbol we can see that the branch language of $\gphors$,
%
...
\begin{aligned}
H_{2}(z)&=
\alpha H_{2}(z)z^2  +\beta z, \qquad\qquad  H_{ij}(z)=0 \quad (i\neq 2),
%\model{A}x&=bx+(1-b)0\\
%\model Bx&=cx+(1-c)0\\
...
The power series 
$a_{H}(z,f,x)=H_2(z)f^2x$ can then be computed by solving the \emph{polynomial equation}
\[
p(z,H_2(z))=0,
\]
...
For instance, it is well-known that the generating functions of order-2 PHORS (which correspond to the so called \emph{indexed grammars}, \cite{DBLP:conf/focs/Aho67}) may fail to be algebraic \cite{DBLP:journals/ita/AdamsFM13}. 
...
Indeed, by relying on the decidability of the \emph{first order theory of the reals} $\Etor$  \cite{Marker2002-jm}, from the algebraicity of $a_{\gphors}(z)$ we will deduce the existence of effective methods to answer both the AST and PAST problem for the corresponding PHORS $\gphors$.
