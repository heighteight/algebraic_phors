...
\end{aligned}
\end{equation}
where $G:(o\multimap o\multimap o)\to \tau$. 
...
Our goal is to extract a generating function from programs like \eqref{eq:introphors1}, i.e.~a formal power series $a_\gphors(z)=\sum_{i=0}^\infty a_iz^i$ capturing the probabilistic termination of $\gphors$. Recall that power series always induce \emph{analytic functions} defined over some open subsets $U\subset \mathbb C$ of the field of complex numbers, and can thus be studied with methods coming from both algebra and complex analysis. For instance, the formal power series $a(x)=\sum_{n=0}^\infty \frac{1}{2^n} x^n$ induces the analytic function $a(x)=\frac{1}{1-\frac{x}{2}}$ defined over the complex numbers of absolute value strictly less than $ 2$.
...
In this way we can finally deduce the probability of termination $a_{\gphors}(1)=
a_{H}(1,1,1)=
\frac{\beta }{1-\alpha }$.
...
For instance, it is well-known that the generating functions of order-2 PHORS (which correspond to the so called \emph{indexed grammars}, cite) may fail to be algebraic. 
...
Indeed, by relying on the decidable \emph{first order theory of the reals} $\Etor$ (cite qualcuno), from the algebraicity of $a_{\gphors}(z)$ we will deduce the existence of effective methods to answer both the AST and PAST problem for the corresponding PHORS $\gphors$.
...
A first natural idea is thus to restrict ourselves to linear, or even affine, programs, as in (Ong et al.). Yet, the PHORS in \eqref{eq:phors1} shows that one can well admit {bounded} forms of non-linearity. 
...
Moreover, thanks to the composability of algebraic power series, it will be possible to \emph{compose} an algebraic PHORS $\gphors_1(w)$, depending on some unbounded parameter $w$, with another algebraic PHORS $\gphors_2$, yielding a new PHORS $\gphors_1(\gphors_2)$, in which $\gphors_2$ is used unrestrictedly. 
