...
If we ignore probabilities, we can consider the \emph{branch language} $\C L(\gphors)$ computed by the PHORS $\gphors$ as follows:
 replace the choice operation $\oplus_{\frac{1}{2}}$ with a binary terminal symbol $c:o\multimap o\multimap o$, and consider the following HORS (i.e.~non-probabilistic recursion scheme) $\gphors'$:
...
Our goal is to extract a generating function from programs like \eqref{eq:introphors1}, i.e.~a formal power series $a_\gphors(z)=\sum_{i=0}^\infty a_iz^i$ capturing the probabilistic termination of $\gphors$. Recall that such power series always induce \emph{analytic functions} defined over some open subsets $U\subset \mathbb C$ of the field of complex numbers, and can thus be studied with methods coming from both algebra and complex analysis. For instance, the formal power series $a(x)=\sum_{n=0}^\infty \frac{1}{2^n} x^n$ induces the analytic function $a(x)=\frac{1}{1-\frac{x}{2}}$ defined over the complex numbers of absolute value strictly less than $ 2$.
...
In we consider the HORS $\gphors'$, we see that, for any $i\in \N$, the term $Gzx$ has a unique way of terminating using $z$ exactly $i$ times; formally, each such reduction yields, in the semantics, a monomial of the form $z^{i}x$. By considering all possible reductions, all such monomials yield then the power series below, interpreting the non-terminal $G$:
\[
a_{G}(z,x)=\sum_{i>0}^{\infty}z^i x
\]
To get back to the original PHORS $\gphors$, we can interpret the ``choice'' constant $c:o\multimap o\multimap o$ as the linear map $c(z)(x,y)=z(\frac{1}{2}x+\frac{1}{2}y)$, depending on some parameter $z$, yields the generating function
%
%We can back to the original PHORS $\gphors$, interpret
\[
a_{F}(z,x)=\sum_{i>0}^{\infty}\frac{1}{2^i}z^i x=\frac{x}{2-z}
\]
Notice that this says precisely that the probability that $Fx$ terminates using $x$ once and making $i>0$ choices is $\frac{1}{2^i}$. 
As $e$ trivially terminates with probability 1, we can then associate the source symbol $S$ with the generating function
\[
a_{S}(z):=a_{F}(z,1)=\sum_{i>0}^{\infty}\frac{1}{2^i}z^i=\frac{1}{2-z},
\]
...
Unfortunately, this apparently smooth way of computing termination probabilities cannot be applied to \emph{all} PHORS.
For instance, it is well-known that the generating functions of order-2 PHORS (which correspond to the \emph{indexed grammars}, cite) may fail to be algebraic. 
In our semantics, the generating function $a_{\gphors}(z)$ of a PHORS $\gphors$ will be determined by a (countably) infinite family of equations, which we might not be able to solve either algebraically or analytically. 
...
Indeed, by relying, as in (Ong et al.), on the decidable \emph{first order theory of the reals} (cite qualcuno), from the algebraicity of $a_{\gphors}(z)$ we will deduce the existence of effective methods to answer both the AST and PAST problem for the corresponding PHORS $\gphors$.
...
But it is possible to can go even beyond that, still preserving decidability.
...
This will allow us to use also infinitely graded types $!_\infty\sigma\multimap \tau$, although in a restricted way. Moreover, thanks to the composability of algebraic power series, it will be possible to \emph{compose} an algebraic PHORS $\gphors_1(w)$, depending on some unbounded parameter $w$, with another algebraic PHORS $\gphors_2$, yielding a new PHORS $\gphors_1(\gphors_2)$, in which $\gphors_2$ is used unrestrictedly. 
...
\paragraph*{Affinization of Bounded PHORS}
