...
We consider a parametric bounded PHORS $\gphors=(\nonterm, \C P,\C R, S)$ and we build from it a $\Gfin$ with paramters $R(\gphors)$ . In the case that $\gphors$ is closed, from $R(\Gfin)$ we can extract a subset of nonterminals that form a (parameter free) $\Gfin$.
\begin{definition}
...
Now we define a translation from $\Ginf$ terms of finite type (i.e terms $t$ that can be typed as $\nonterm\mid\C P\mid\Delta\vdash \vdash t: \varphi $) to $\Gfin$ terms $R(t)$ that can be typed as $\nonterm' \mid \Delta \vdash R(t): \phi$. 
