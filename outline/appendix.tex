...
Take a standard non idempotent intersection type system, for example:
$$
...
\end{lemma}
\begin{proof} 
	If we have a typing of $ x_1: \sigma_1 \dots  x_n: \sigma_n \vdash t:  \1 $, $t$ has an head normal form, then take any head normal form $L$ of $t$ (which must satisfy  $ x_1: \sigma_1 \dots  x_n: \sigma_n \vdash L:  \1 $, hence also $ x_1:\1 \dots x_n:\1\vdash L:\1$) . Then either $L = x$ or $L=e$. In the first case, we can only derive $x_i:[1] \vdash x_i:1$, in the second we just have $x_i:\emptyset \vdash e:1$\\
\end{proof}
Then we get
...
\begin{lemma}[Correctness of $\Gfin$ for ground terms]
	...
	Given a term $\nonterm\mid\Delta\vdash t: \1$, let $s \bydef \model{t: \1^n}_{}^{\Rsemiring} $ be its $\Qrelkleisli \Rsemiring$interpretation, which consists of a single power series. Then, $s_\sigma\big\vert_{\model{\nonterm}+ \model{\Delta}} \in \fpp{\Rsemiring}{\model{\nonterm}+ \model{\Delta}}$ is a polynomial. Moreover, for each $f:_k \psi \in \Delta$, the total degree of $s$ in the variables $x_{f:_k \psi}$ is at most $k$
\end{lemma}
...
We consider a parametric bounded PHORS $\gphors=(\nonterm, \C P,\C R, S)$ and we build from it a $\Gfin$ PHORS with paramters $R(\gphors)$ . In the case that $\gphors$ is closed, from $R(\Gfin)$ we can extract a subset of nonterminals that form a (parameter free) $\Gfin$.
\begin{definition}
...
\end{definition}
Now we define a new reduced PHORS, with an enlarged set $\nonterm'$ of non-terminals
...
Now we define a translation from applicative terms (i.e terms $t$ that can be typed as $\nonterm \mid \Delta \mid \mathcal{P} \vdash t: \tau $ only using  axioms and rules $@, @_\infty$) to a reduced applicative terms (i.e applicative terms $t'$ that can be typed as $\nonterm' \mid \Delta \mid \mathcal{P} \vdash t': \tau$ \textbf{without using the $@_\infty$ rule or any axiom of infinite type}).
First we prove the following:
\begin{lemma}
	If $\mathcal{N}|\Delta| \mathcal{P} \vdash t: \tau$ is an applicative term of type $!_{\infty} \psi_{1} \multimap \tau$, then it is of the form $L t_{1} \ldots t_{k}$, where $L \in \mathcal{N}$ and $t_{i} \in \mathcal{N} \cup \mathcal{P}$.
\end{lemma}
Then we get: 
\begin{lemma}
	For every applicative term $\mathcal{N}|\Delta| \mathcal{P} \vdash t: \tau$, there exists a reduced applicative terms $\mathcal{N} \cup \mathcal{F}|\Delta| \mathcal{P} \vdash t: \tau$ such that $t=t^{\prime}\left[f_{1} / r_{1} \ldots f_{l} / r_{l}\right]$ and each $r_{i}$ is of the form $L L t_{1} \ldots t_{k}$, where $L \in \mathcal{N}$ and $t_{i} \in \mathcal{N} \cup \mathcal{P}$. If $\tau$ is a finite type, then $\mathcal{F}$ only contains finite types.
\end{lemma}
\begin{proof}
If $\mathcal{N}|\Delta| \mathcal{P} \vdash t: \tau$ is an axiom of finite type, $t^{\prime}=t$. If $\mathcal{N}|\Delta| \mathcal{P} \vdash L: \tau, \mathcal{N}(L)= !_{\infty} \psi \multimap \tau$, then $t=f_{1}, \mathcal{F}=\left\{f_{1}:!_{\infty} \psi \multimap \tau\right\}$ and $r_{1}=L$.\\
...
If $t=t_{1} t_{2}: \tau$ and the last step was an infinitary application, then $t_{1}$ is of the form $L s_{1} \ldots s_{n}$ and $t_{2} \in \mathcal{N} \cup P$ : then take $t^{\prime}:=f_{1}, \mathcal{F}=\left\{f_{1}: \tau\right\}$ and $r_{1}=L s_{1} \ldots s_{n} t_{2}: \tau$\\
\end{proof}
Now we define a translation from applicative terms typable in $\Ginf$ to terms typable in $\Gfin$: 
\begin{definition}
 Let $\mathcal{N}|\Delta| \mathcal{P} \vdash t: \psi$ be an applicative term of finite type. Then we define its translation $R(t)$ to be the following finitary applicative term (i.e a term $t^{\prime}$ that can be typed in the finitary system as $\mathcal{N}^{\prime} \mid \Delta \cup \mathcal{P} \vdash t^{\prime}: \psi$, where $\operatorname{dom}\left(\mathcal{N}^{\prime}\right)=N^{\prime}$ ). Given $t$, take $t^{\prime}$ as defined in the last lemma. For each $r_{i}=L t_{1} \ldots t_{n}$ (which must be of finite type, as $t$ is such), define $R\left(r_{i}\right):=\tilde{L}_{\left[t_{1} \ldots t_{n}\right]} \in N^{\prime}$. Then define $R(t)$ to be the result of replacing every $f_{i}$ in $t^{\prime}$ by $R\left(r_{i}\right)$. Replace every type binding $f_{i}: \psi$ in $\mathcal{F}$ with a binding $R\left(r_{i}\right): \psi$ and every type binding $L: \psi \in \mathcal{N}$ by a type binding $L_{\square}: \psi$; the set of all the just defined type bindings is $\mathcal{N}^{\prime}$	
\end{definition}
...
\begin{definition}
	We say that a nonterminal $L$ depends over a nonterminal $G$ if (1) in the definition of $L$ $G$ occurs (2) Inductively, in the definition of $L$ a non terminal $G'$ occurs that depend over $G$ 
\end{definition}
\begin{lemma}
	If $\gamma$ contains free parameter $p_1, \dots p_j$ and the definition of $\tilde L_{\gamma}$
	is $\tilde L_\gamma x_1 \dots x_j = R(t_L[\vec g / \gamma]) $, then $R(t_L[\vec g / \gamma])$ contains non-terminals $G_\gamma'$ where $\gamma'$ contains free paramters amongst $p_1, \dots p_j$.
\end{lemma}
\begin{proof}
	The only free variables in $t_L$ were the $g_1, \dots g_n$ and the $x_1, \dots x_j$. Since there was the free parameters in $\gamma$ were $p_1, \dots p_j$, $FV(t_L[\vec g / \gamma]) \subseteq \{x_1, \dots x_n, p_1, \dots p_j\}$: it does not contains only $p_1, \dots p_j$ as parameters. 
\end{proof}
...
\begin{equation*}
	...
	\end{cases}
\end{equation*}
\subsection{About the composition of PHORS}
