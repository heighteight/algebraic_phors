...
\begin{definition}
	For each type $\varphi$, we define by induction a set ${\Com}(\varphi)$ such that $\model{\varphi} \subseteq \Com(\phi) \subseteq \model{\st{\varphi}}$:
	\begin{enumerate}
...
\begin{lemma}
	Given a term $\nonterm\mid\Delta\vdash t: \varphi$, let $s_{\model{\st{\varphi}}} \bydef \model{t: \st{\varphi}}^{\Rsemiring} $ be its $\Qrelkleisli \Rsemiring$interpretation. We have that:
	\begin{enumerate}
		\item For each $\sigma \in \model{\st{\varphi}} \setminus \Com(\model{\varphi})$, 
		\begin{equation}
...
\begin{lemma}
	Given a term $$\nonterm\mid\emptyset \vdash : \lambda x_1 \dots x_n.t: \varphi$$ with $\varphi = !_{k_1} \psi_1 \multimap \dots !_{k_n} \psi_n \multimap \1$, let $s_{\model{\st{\varphi}}} \bydef \model{t: \st{\varphi}}^{\Rsemiring} $ be its $\Qrelkleisli \Rsemiring$interpretation. Then:
	\begin{enumerate}
		\item If $\sigma \not \in \model{\st{\varphi}} \setminus \model{\varphi}$, then $s_\sigma\Big\vert_{\model{\nonterm}}=0$
		\item If  $\sigma \in \model{\st{\varphi}}$, then $s \Big\vert_{\model{\nonterm}}$ is a polynomial. 
	\end{enumerate}
\end{lemma}
\begin{proof}
